...
Twenty-five articles coded EVENT under the ``Novichok'' query, with the coder's event place and object. Fourteen, all dated February 2024, report the death in custody of Alexei Navalny, reported as a suspected poisoning; the other eleven report other suspected poisonings, including three from February 2026. None reports a new use of Novichok.\\[4pt]
...
\section*{S13. Treatment of the zero-count cell}
Block event shares are post-stratified onto six cells formed by block and coder A's EVENT-versus-rest decision. The table gives, for each cell, the number of agreed human events, the number of agreed articles drawn, the cell's size in the gold set, and a 95\% Jeffreys interval on the cell's own share.\\[4pt]
\begin{tabular}{lrrrl}\toprule Cell & Human events & $n$ agreed & $N$ gold & Jeffreys 95\% \\\midrule
Industrial, coder A EVENT & 23 & 32 & 418 & 0.549--0.851 \\
Industrial, coder A non-EVENT & 2 & 42 & 549 & 0.010--0.144 \\
Security, coder A EVENT & 18 & 79 & 95 & 0.146--0.329 \\
Security, coder A non-EVENT & 4 & 140 & 805 & 0.010--0.066 \\
Conventional, coder A EVENT & 26 & 28 & 409 & 0.790--0.985 \\
Conventional, coder A non-EVENT & 0 & 39 & 591 & 0.000--0.062 \\
\bottomrule\end{tabular}\\[6pt]
The conventional non-EVENT cell holds no human event. Any resampling scheme that draws within cells returns that cell's share as zero in every resample, so the cell contributes no variance and the block share cannot exceed coder A's value; the coincidence between the two in Table 6 of the main text follows from the zero count rather than from agreement. Carrying the Jeffreys limits for each cell through the post-stratification weights gives block envelopes of 24.3--45.0\% (industrial), 2.4--9.4\% (security) and 32.3--43.9\% (conventional). These take both cells' limits at once and are therefore conservative envelopes rather than 95\% intervals, and they resample nothing: the security envelope is narrower than that block's query-clustered interval for that reason, not because the zero-cell treatment tightened anything. The conventional upper limit is then 43.9\% rather than 40.9\%, and the cap is removed. These figures assume independence within cells and are therefore not a substitute for the query-clustered intervals reported in the main text, which match the unit of analysis; they are given to show that the cap is an artefact of a zero count and that no conclusion rests on it. The same zero count makes coder A's conventional sensitivity of 1.00 a point estimate without an upper constraint; taking the Jeffreys upper limit on the missed-event cell gives a lower bound of 0.91.

\section*{S14. Recall of the registry substance matching}
The strict recode of Section 4.5 removes false matches. This section looks for the opposite error. For each of the six substances we searched the incident description and cause fields of all 366 records in the overlap window under an expanded set of names, including anhydrous and waste forms and salt names, and adjudicated every record that the substance-field rule had not already matched.\\[4pt]
{\small\setlength{\tabcolsep}{4pt}\begin{tabular}{llp{6.5cm}p{1.9cm}}\toprule Substance & Date & Record & Verdict \\\midrule
NH$_3$ & 2025-03-10 & Substance field nitric acid (60\%); ammonia water named only as the delivery being made when the nitric acid fell & Not a release \\
H$_2$SO$_4$ & 2022-12-14 & Substance field sodium hydroxide; sulfuric acid plant damaged by the explosion but not released & Not a release \\
Toluene & 2023-01-26 & RAW C9+C stream: solvent naphtha, indene, vinyltoluene, styrene. Vinyltoluene is methylstyrene & Different compound \\
HNO$_3$ & 2025-02-27 & Ammonium nitrate sacks fell from a truck & Salt, excluded by the strict rule \\
HF & 2024-08-16 & Substance field ammonium bifluoride; description records HF residue in a pipe under replacement, with an injury & \textbf{Miss} \\
\bottomrule\end{tabular}}\\[6pt]
One of the five candidates is a release of the substance the rule was looking for. Adding it gives recall 0.92 for hydrogen fluoride and 1.00 for the other five substances, 142 of 143 releases. The log--log slope moves from $-0.036$ ($p = 0.90$) to $-0.023$ ($p = 0.94$), and the hypothesis of a slope of 1 is still rejected ($t(4) = 3.6$, $p = 0.023$). The audit code and its output are deposited as \texttt{analyze\_22\_registry\_recall.py} and \texttt{registry\_recall.json}.
