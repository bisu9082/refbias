...
import json,re,sys
F=json.load(open('/home/claude/rb/Step5r/v8_final_numbers.json'))
H=json.load(open('/home/claude/rb/Step5r/human_validation.json'))
...
for disp,b in [('Industrial','industrial'),('Security','security'),('Conventional','conventional')]:
    v=F['blocks'][b]
    m=re.search(disp+r' & (\d+) \((\d+)\) & ([0-9.]+) & ([0-9.]+) & ([0-9.]+) & ([0-9.]+) \[([0-9.]+), ([0-9.]+)\] & ([0-9.]+)--([0-9.]+) & ([0-9.]+) \[([0-9.]+), ([0-9.]+)\]',tex)
    ...
    exp=['%d'%v['n_drawn'],'%d'%v['n_consensus'],'%.2f'%v['TPR'],'%.2f'%v['FPR'],'%.1f'%(100*v['pE_coderA']),
         '%.1f'%(100*v['pE_human']),'%.1f'%(100*v['ci95'][0]),'%.1f'%(100*v['ci95'][1]),
         '%.1f'%min(rng),'%.1f'%max(rng),'%.1f'%v['A'],'%.1f'%v['A_ci'][0],'%.1f'%v['A_ci'][1]][:12]
    for i,lab in enumerate(['n_drawn','n_cons','TPR','FPR','pE_A','pE_H','lo','hi','rng_lo','rng_hi','A','A_lo']):
...
# amplification prose
chk('A sec',r'gives \$A = ([0-9.]+)\$ \([0-9.]+--[0-9.]+\) for the security block','%.1f'%F['blocks']['security']['A'])
chk('A ind',r'for the security block, ([0-9.]+) \([0-9.]+--[0-9.]+\) for the industrial block','%.1f'%F['blocks']['industrial']['A'])
chk('A conv',r'for the industrial block and ([0-9.]+) \([0-9.]+--[0-9.]+\) for the conventional block','%.1f'%F['blocks']['conventional']['A'])
...
# domestic
d=F['domestic']
chk('domestic human',r'the domestic share of security coverage is ([0-9.]+)\\% \(0\.8--4\.6\)','2.5')
