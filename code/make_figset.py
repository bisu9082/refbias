...
W=os.path.expanduser('~/rbwork'); OUT=W+'/out'; R=os.path.expanduser('~/mnt/claude_research/refbias/data'); F=OUT+'/figset'; os.makedirs(F,exist_ok=True)
# ── style
...
# (c) drone growth
axc=fig.add_subplot(gs[1,1]); dr=cp[cp.qid=='Q23'].groupby('year')['n'].sum()
axc.bar(dr.index.astype(str),dr.values,color=BLK['비CBRN'],width=0.66,edgecolor='white')
for i,v in enumerate(dr.values): axc.text(i,v+350,f'{v:,}',ha='center',fontsize=15,color=INK)
axc.text(5,19199+2100,'×54 in five years',ha='center',fontsize=17,fontweight='bold',color=INK)
axc.set_ylabel('Articles: "drone attack"',fontsize=18); axc.set_xlabel('Year  (2021: from Sep 9;  2026: to Sep 8)',fontsize=18); axc.set_ylim(0,23500); axc.tick_params(labelsize=16)
...
ax.axhline(1,ls='--',c=INK2,lw=1); ax.text(2.48,1.25,'A = 1 (no amplification)',fontsize=13,color=INK2,ha='right',va='bottom',bbox=dict(fc='white',ec='none',pad=1.5)); ax.set_ylabel('Amplification factor A'); ax.set_ylim(0,15); ax.set_xlim(-0.5,2.5); ax.text(0.02,0.97,'error bars: 95% bootstrap CI\n(10,000 resamples)',transform=ax.transAxes,ha='left',va='top',fontsize=13,color=INK2); P(ax,'b',dy=0.040)
# (c) strip by block
ax=fig.add_subplot(gs[1,0]); rng=np.random.default_rng(1)
for i,bk in enumerate(BORD):
    v=aq.loc[aq.block==bk,'증폭계수'].values; xj=i+rng.uniform(-0.16,0.16,len(v))
...
ax.set_yscale('log'); ax.set_xticks(range(3)); ax.set_xticklabels([f"{BEN[x].replace(' terror',chr(10)+'terror')}\nmedian {np.median(aq.loc[aq.block==x,'증폭계수']):.1f}" for x in BORD]); ax.set_ylabel('Query-level amplification A (log)'); ax.set_xlim(-0.6,2.6)
ax.text(0.02,0.96,'Kruskal–Wallis H(2) = 11.08, p = 0.0039, η² = 0.34\nSecurity vs Industrial  p = 0.009, r = 0.70\nSecurity vs Conventional  p = 0.007, r = 0.73\nIndustrial vs Conventional  p = 0.13 (n.s.)',transform=ax.transAxes,va='top',fontsize=15,color=INK,bbox=dict(boxstyle='round,pad=0.4',fc='white',ec=GRID))
P(ax,'c')
...
ax=fig.add_subplot(gs[0,0]); a=pd.read_csv(R+'/anchor_rates.csv'); aen={'노비촉':'Novichok\n(Skripal / Navalny)','사린가스 공격':'Sarin attack\n(Aum / Tokyo 1995)','방사능 유출':'Radiation leak\n(Fukushima / Chernobyl)','탄저균 테러':'Anthrax terror\n(2001 US letters)','불산 누출':'HF leak\n(2012 Gumi)','염소 가스 누출':'Cl2 gas leak','황산 유출':'H2SO4 spill','질산 누출':'HNO3 leak'}
a['en']=a['query'].map(aen); a['grp']=np.where(a['앵커언급률']>0.2,'anchor','control'); a=a.sort_values('앵커언급률')
cols=[CAT['EVENT'] if g=='anchor' else CAT['IRRELEVANT'] for g in a['grp']]
ax.barh(range(len(a)),a['앵커언급률']*100,color=cols,height=0.7,hatch=['' if g=='anchor' else '///' for g in a['grp']],edgecolor='white')
ax.set_yticks(range(len(a))); ax.set_yticklabels(a['en'],fontsize=14.5); ax.grid(axis='x'); ax.grid(False,axis='y')
for i,(v,n) in enumerate(zip(a['앵커언급률'],a['n'])): ax.text(v*100+1.5,i,f'{v*100:.1f}%  (n={n})',va='center',fontsize=14,color=INK)
ax.set_xlim(0,105); ax.set_xlabel('Articles citing the anchor event (%)')
ax.legend(handles=[Patch(color=CAT['EVENT'],label='CBRN / rare-hazard queries'),Patch(facecolor=CAT['IRRELEVANT'],hatch='///',edgecolor='white',label='Same-block controls (Gumi 2012 as anchor)')],loc='lower center',bbox_to_anchor=(0.60,1.0),fontsize=13.5,ncol=1); P(ax,'a',off=0.20,dy=0.075)
# (b) CARIS
...
xs,ys=np.log10(cm['공식사건수']),np.log10(cm['기사당사건비']); sl,ic=np.polyfit(xs,ys,1); rr=np.corrcoef(xs,ys)[0,1]
xx=np.linspace(0.9,1.8,50); ax.plot(10**xx,10**(sl*xx+ic),'--',c=INK2,lw=1.6,zorder=1)
ax.scatter(cm['공식사건수'],cm['기사당사건비'],s=260,color=CAT['EVENT'],edgecolor='white',lw=1.5,zorder=3)
for _,r in cm.iterrows(): ax.annotate(lab[r['query']],(r['공식사건수'],r['기사당사건비']),xytext={'질산 누출':(-16,12),'불산 누출':(10,-20)}.get(r['query'],(9,7)),textcoords='offset points',fontsize=17)
...
ax.set_xticks([10,20,30,50]); ax.set_yticks([3,5,10,20]); ax.set_xlim(8,70); ax.set_ylim(2.3,28)
ax.set_xlabel('Official incidents (CARIS, 2021-09 to 2025-04)'); ax.set_ylabel('Articles per official incident')
ax.text(0.97,0.97,f'slope = {sl:.2f}  (r = {rr:.2f}, n = {len(cm)})\ncoverage ∝ incidents$^{{{1+sl:+.2f}}}$',transform=ax.transAxes,fontsize=14.5,color=INK,ha='right',va='top',bbox=dict(boxstyle='round,pad=0.4',fc='white',ec=GRID)); P(ax,'b',off=0.075,dy=0.075)
...
# (c) confusion A vs B
ax=fig.add_subplot(gs[1,0]); A_=pd.concat([pd.read_csv(f) for f in sorted(__import__('glob').glob(os.path.expanduser('~/mnt/claude_research/refbias/Step4/labels_A/*.csv')))],ignore_index=True)
B_=pd.concat([pd.read_csv(f) for f in sorted(__import__('glob').glob(os.path.expanduser('~/mnt/claude_research/refbias/Step4/labels_B/*.csv')))],ignore_index=True)
...
ax.set_xticks(range(5)); ax.set_xticklabels(C,fontsize=14.5,rotation=0); ax.set_yticks(range(5)); ax.set_yticklabels(C,fontsize=13.5); ax.set_xlabel('Labeler B (n = 600)\nκ = 0.790 [0.751, 0.829]   ·   EVENT vs rest κ = 0.875',fontsize=16); ax.set_ylabel('Labeler A'); ax.tick_params(length=0)
